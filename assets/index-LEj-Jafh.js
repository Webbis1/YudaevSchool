...
class Tabs {
  ...
  panels;
  indicator;
  constructor(selector3 = ".tabs") {
    this.tabs = document.querySelector(selector3);
...
class CasesSlider {
  sliders = /* @__PURE__ */ new Map();
  initTabSlider(tabId) {
    const panel = document.querySelector(`.tabs__panel[data-tab="${tabId}"]`);
    if (!panel) return;
    const swiperEl = panel.querySelector(".swiper");
    const paginationEl = panel.querySelector(
      ".swiper__pagination"
    );
    const prevEl = panel.querySelector(".swiper__arrow_prev");
    const nextEl = panel.querySelector(".swiper__arrow_next");
...
class StudentResults {
  ...
  calcInitialHeight() {
    const item = this.inner.children[0];
    if (!item) return 0;
    const gap = 95;
...
const initCircleModule = () => {
  ...
  const cards = gsapWithCSS.utils.toArray(".real-pros__card");
  const finalDeg = -30;
  const circleTL = gsapWithCSS.timeline({
    scrollTrigger: {
      trigger: "#real-pros",
      start: "top top",
      end: `+=${1e3}`,
  ...
  cards.forEach((card, i) => {
    const startAngleDeg = i % 2 == 0 ? startDeg + i * -7 : startDeg + 180 + i * -7;
...
const initBurgerMenu = () => {
  ...
  const burgerTextMobile = burgerMobile?.querySelector(
    ".header__burger-text"
  );
  ...
  document.addEventListener("click", (e) => {
    ...
    if (isCoursesVisible && !coursesBlock?.contains(target) && !burgerPc?.contains(target)) {
      burgerIconPc?.classList.remove("active");
      blurBackground?.classList.remove("active");
      animateCourses();
    }
    if (isMobileContentVisible && !mobileContent?.contains(target) && !burgerMobile?.contains(target)) {
...
const initImageBlurEffect = () => {
  ...
  const parentElement = document.querySelector(".advantages");
  gsapWithCSS.to(imageContainer, {
    filter: "blur(10px)",
    scrollTrigger: {
...
const initMobileCoursesNavigation = () => {
  ...
  const mobileCourses = document.querySelector(
    ...
    gsapWithCSS.to(mobileCourses, {
      ...
      onUpdate: () => {
        console.log("Текущие координаты:", {
          x: gsapWithCSS.getProperty(mobileCourses, "x"),
      ...
      onComplete: () => {
        ...
        console.log(
          ...
          gsapWithCSS.getTweensOf(mobileCourses)
        );
      }
    });
  };
  ...
  gsapWithCSS.set(mobileCourses, { clearProps: "transform" });
  gsapWithCSS.set(mobileCourses, {
    x: "300%",
...
//# sourceMappingURL=index-LEj-Jafh.js.map
